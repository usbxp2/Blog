...
def UploadFile(request):
    ...
    #取得上传的文件的原始名称
    upload_original_name,upload_original_ext=os.path.splitext(upload_file_name)     # 这里取得上传文件的原文件名！！！！！！！！！！！！！！！！！！！！！！！！
    ...
    path_format_var.update({
        "basename":upload_original_name,   #这里设置basename,就是命名规则的原文件名！！！！！！！！！！！！！
    ...
    if state=="SUCCESS":
        if action=="uploadscrawl":
            state=save_scrawl_file(request,os.path.join('media',OutputFile))    #上传路径修改
        else:
            #保存到文件中，如果保存错误，需要返回ERROR
            state,url=save_upload_file(file,os.path.join('media',OutputFile))   #sae,图片上传路径url修改为domainname/media/文件名
